        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col gap-4 mb-6">
                <div>
                    ...
                    <p class="mt-2 text-sm text-gray-600">Find and book the perfect room for your next meeting.</p>
                </div>
                ...
                <form method="get" class="flex flex-wrap items-center gap-3 bg-white shadow-sm ring-1 ring-gray-200 rounded-2xl p-4 text-sm">
                    ...
                    <select name="sort" class="rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <option value="favorites" @selected(($sort ?? '')==='favorites')>Favorites only</option>
                    </select>

                    <label class="inline-flex items-center gap-2 text-sm font-medium text-gray-700">
                        <input type="checkbox" name="saved" value="1" @checked(($saved ?? false)) class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                        Favorites only
                    </label>
                    ...
                    <div class="relative flex-1 min-w-[220px] sm:min-w-[280px]">
                        ...
                        <input name="q" value="{{ $q }}" 
                            ...
                            class="block w-full pl-10 pr-3 py-2.5 rounded-md border border-gray-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                    </div>
                            ...
                            <div class="p-6 flex-grow">
                                <div class="flex justify-between items-start mb-4">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <form method="POST" action="{{ $room->is_favorite ? route('client.favorites.destroy', $room) : route('client.favorites.store', $room) }}">
                                            @csrf
                                            @if($room->is_favorite)
                                                @method('delete')
                                            @endif
                                            <button type="submit" class="flex items-center gap-1 text-xs font-semibold {{ $room->is_favorite ? 'text-yellow-600' : 'text-gray-500 hover:text-gray-700' }}">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="{{ $room->is_favorite ? 'currentColor' : 'none' }}" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.957a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.37 2.449a1 1 0 00-.364 1.118l1.286 3.957c.3.921-.755 1.688-1.54 1.118l-3.37-2.449a1 1 0 00-1.176 0l-3.37 2.449c-.784.57-1.838-.197-1.539-1.118l1.285-3.957a1 1 0 00-.364-1.118L2.065 9.384c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.95-.69l1.284-3.957z" /></svg>
                                                {{ $room->is_favorite ? 'Saved' : 'Save' }}
                                            </button>
                                        </form>
                                    </div>
                                </div>
                                ...
                                @if($room->equipment)
                                    <div class="mt-2 text-xs text-gray-500">Equipment: {{ implode(', ', $room->equipment ?? []) }}</div>
                                @endif
                                @if($room->usage_rules)
                                    <div class="mt-1 text-xs text-gray-500">Rules: {{ Str::limit($room->usage_rules, 120) }}</div>
                                @endif
                            </div>
